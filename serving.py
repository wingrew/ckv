...
class BenchmarkMetrics:
    ...
    # Arithmetic mean of independently calculated per-request throughput
    mean_per_request_prefill_throughput: float
    mean_per_request_decode_throughput: float
...
def calculate_metrics(
    ...
    per_request_prefill_throughputs: List[float] = []
    per_request_decode_throughputs: List[float] = []
    ...
    for i in range(len(outputs)):
        if outputs[i].success:
            ...
            if output_len > 1:
                decode_duration = outputs[i].latency - outputs[i].ttft
                if decode_duration > 0:
                    tpots.append(decode_duration / (output_len - 1))
                    per_request_decode_throughputs.append(
                        (output_len - 1) / decode_duration
                    )
            if outputs[i].ttft > 0:
                per_request_prefill_throughputs.append(
    ...
    metrics = BenchmarkMetrics(
        ...
        / dur_s,
        mean_per_request_prefill_throughput=np.mean(
            per_request_prefill_throughputs or 0
        ),
        mean_per_request_decode_throughput=np.mean(
            per_request_decode_throughputs or 0
        ),
...
async def benchmark(
    ...
    if not is_embedding:
        print("{s:{c}^{n}}".format(s="Per-Request Throughput", n=50, c="-"))
        print(
            "{:<40} {:<10.2f}".format(
                "Mean prefill throughput (tok/s):",
                metrics.mean_per_request_prefill_throughput,
            )
        )
        print(
            "{:<40} {:<10.2f}".format(
                "Mean decode throughput (tok/s):",
                metrics.mean_per_request_decode_throughput,
            )
        )
    if accept_length:
    ...
    if (
        ...
        result = {
            ...
            "mean_per_request_prefill_throughput": (
                metrics.mean_per_request_prefill_throughput
            ),
            "mean_per_request_decode_throughput": (
                metrics.mean_per_request_decode_throughput
            ),
...
def cli_main():
    ...
    parser.add_argument(
        ...
        default="dataset",
        help=(
            "Dispatch OpenAI-format requests in dataset order or a TTFT-oriented "
            "input-length order."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Reuse OpenAI dataset token IDs computed during loading when the "
            "chat backend supports pre-computed input_ids."
        ),
    )
    parser.add_argument(
        "--agentic-max-turns",
