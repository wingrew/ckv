...
class CacheAgnosticPolicy(Enum):
    ...
    SJF = "sjf"  # shortest remaining prefill first
...
class SchedulePolicy:
    ...
    def calc_priority(
        ...
        if isinstance(policy, CacheAwarePolicy):
            ...
            if policy == CacheAgnosticPolicy.FCFS:
                ...
                SchedulePolicy._sort_by_shortest_remaining_prefill(waiting_queue)
    ...
    @staticmethod
    def _sort_by_shortest_remaining_prefill(waiting_queue: List[Req]) -> None:
        """Minimize mean TTFT by completing shorter remaining prefills first."""
        waiting_queue.sort(
            key=lambda req: (
                # A retracted decode request has already produced its first
                # token. Keep fresh requests ahead of it so recomputation never
                # delays another request's TTFT.
                bool(req.output_ids),
                max(0, len(req.origin_input_ids) - req.num_matched_prefix_tokens),
            )
        )
