...
class Scheduler(
    ...
    def _get_new_batch_prefill_raw(
        ...
        sjf_multi_request_cap = self.server_args.sjf_multi_request_prefill_tokens
        sjf_multi_request_max_running = (
            self.server_args.sjf_multi_request_prefill_max_running_requests
        )
        sjf_multi_request_active = (
            self.schedule_policy == "sjf"
            and sjf_multi_request_cap is not None
            and sjf_multi_request_cap > 0
            and len(self.waiting_queue)
            >= self.server_args.sjf_multi_request_prefill_threshold
            and (
                sjf_multi_request_max_running is None
                or running_bs < sjf_multi_request_max_running
            )
        )
        if sjf_multi_request_active:
            max_prefill_tokens = min(max_prefill_tokens, sjf_multi_request_cap)
            if chunked_prefill_size is not None:
                chunked_prefill_size = min(
                    chunked_prefill_size, sjf_multi_request_cap
                )
            sjf_batch_max_requests = (
                self.server_args.sjf_multi_request_prefill_max_requests
            )
            if sjf_batch_max_requests is not None and sjf_batch_max_requests > 0:
                prefill_max_requests = (
                    sjf_batch_max_requests
                    if prefill_max_requests is None
                    else min(prefill_max_requests, sjf_batch_max_requests)
                )
