...
class ServerArgs:
    ...
    sjf_multi_request_prefill_tokens: A[
        Optional[int],
        Arg(
            help=(
                "For SJF scheduling, cap the prefill token budget when more than "
                "one request is waiting. This reduces head-of-line TTFT without "
                "slowing the common single-request prefill path."
            ),
            type_parser=human_readable_int,
        ),
    ] = None
    sjf_multi_request_prefill_threshold: A[
        int,
        "The minimum waiting-queue length that activates --sjf-multi-request-prefill-tokens.",
    ] = 2
    sjf_multi_request_prefill_max_requests: A[
        Optional[int],
        "Maximum requests in an SJF multi-request prefill batch.",
    ] = 2
    sjf_multi_request_prefill_max_running_requests: A[
        Optional[int],
        "Only activate SJF multi-request prefill limits below this running batch size.",
    ] = None
    ...
    enable_prefill_priority: A[
        ...
        "ranks stay idle while any rank is running prefill, reducing time to first "
        "token at the cost of inter-token latency.",
    ] = False
